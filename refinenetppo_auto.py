...
class PPOTransition:
    ...
    gt_step_mode: float = 0.0      # 新增
    gt_terminate: float = 0.0      # 新增
...
class RefineNetActorCritic(nn.Module):
    ...
    def __init__(self, num_actions: int, input_size: int = 128,
        ...
        # Step mode decision head: 预测是否应该切换到fine mode
        ...
        # Termination decision head: 预测是否应该停止
...
class PPOAgent:
    def __init__(
        ...
        terminate_pos_weight: float = 5.0,  # 新增
        ...
        for _ in range(epochs):
            for batch in buffer.get_batches(batch_size, self.device):
                ...
                terminate_loss = F.binary_cross_entropy_with_logits(
                    ...
                    gt_terminates, 
                    reduction='mean',
                    pos_weight=torch.tensor([2.0], device=self.device)  # 2倍惩罚早停
...
def train():
    ...
    TARGET_IDS = (7, 37)  # Fixed ID combination (curriculum start)
    ...
    env.target_augmenter = TargetImageAugmenter(
        ...
        apply_contour_aug=True,
        contour_aug_prob=0.2,  # 50% chance to augment contours
        skeleton_dilate_range=(1, 5),
        
        # Depth augmentation  
        apply_depth_aug=True,
        depth_aug_prob=0.5,  # 50% chance to augment depth
        occluder_probability=0.6,
        mask_dilate_prob=0.7,  # 50% dilation, 50% erosion
    ...
    model = RefineNetActorCritic(
        ...
        use_bn=False,
        rot_rep="6d",
        pretrained_path="./patient3_0805_fold1_best_val.pth",
        freeze_encoder=True
    ).to(device)
    ...
    ppo_agent = PPOAgent(
        ...
        bc_coef=0.0,
        decision_coef=0.5,
        terminate_pos_weight=5.0,  # 5倍惩罚早停
        device=device
    )
    # Optional: resume from checkpoint
    resume_path = "checkpoints/refinenet_ppo_18000.pt"
    ...
    for update in range(num_updates):
        ...
        # 在 train() 函数中，decision accuracy计算之后添加
        if len(buffer.transitions) > 0:
            ...
            tp = ((term_pred_binary == 1) & (term_gt_binary == 1)).sum()  # True Positive: 正确预测停止
            fp = ((term_pred_binary == 1) & (term_gt_binary == 0)).sum()  # False Positive: 错误停止（晚停）
            fn = ((term_pred_binary == 0) & (term_gt_binary == 1)).sum()  # False Negative: 错误继续（早停）⚠️
            tn = ((term_pred_binary == 0) & (term_gt_binary == 0)).sum()  # True Negative: 正确预测继续
            ...
            print(f"  ")
            print(f"  Key Metrics:")
            print(f"    - Precision (停止时正确率): {tp/(tp+fp)*100 if (tp+fp)>0 else 0:.1f}%")
            print(f"    - Recall (识别出应停比例): {tp/(tp+fn)*100 if (tp+fn)>0 else 0:.1f}%")
            print(f"    - FN Rate (早停率): {fn/(fn+tp)*100 if (fn+tp)>0 else 0:.1f}% ⚠️ 越低越好")
            # ========= 结束 =========
